...
include("PHP/create_post.php");?>
<!DOCTYPE html>
<html>
<head lang="en">
    <meta charset="UTF-8">
    <title>Tiny Darya || Add post</title>
...
<div id="input-form" class="small-12 columns">
    <form id="form" action="" method="post">
        ...
        <!-- Paragraph -->
        <p id="heading">Add new post:</p>

        <!-- Title -->
        <div>
            <label for="title">Title:</label>
            <input id="title" class="input" name="title" type="text"/>
        </div>

        <!-- Text -->
        <div>
            <label for="text">Text:</label>
            <textarea id="text_area" class="input" name="text" cols="45" rows="20"></textarea>
        </div>
        ...
        <div>
            <fieldset>
                ...
                <?php
                    ...
                    if (!$tags) {
                        echo "Failed to fetch tags: " . mysqli_error($con);
                    } else {
                        while ($tag = mysqli_fetch_assoc($tags)) { ?>
                            <input type="checkbox" name="hashtags[<?php echo $tag['id']; ?>]" value="<?php echo $tag['id']; ?>" /><?php echo $tag['name']; ?><br />
                        <?php }
                    }
                ?>
            </fieldset>
